...
현재 버전 `0.4.0`은 기능과 UI를 검증하는 프리뷰입니다. 배포용 실행 파일에는 아직 코드 서명이 적용되지 않아 Windows가 SmartScreen 경고를 표시할 수 있습니다. 소스 코드를 직접 빌드하거나 이 저장소의 공식 Releases에서 받은 파일만 사용하는 것을 권장합니다.
...
- 세션·방문 기록의 500ms 백그라운드 저장, 재시도와 비모달 저장 실패 상태 표시
...
이 공통 검증은 솔루션 복원, 단 한 번의 Release/x64 빌드, Core 테스트, 전경을 획득하지 않는 STA App 동작 테스트, 검증된 출력의 배포 staging·라이선스 검사와 `git diff --check`를 순서대로 실행합니다. UI 테스트는 외부 입력이나 네트워크를 사용하지 않고 실제 컨트롤의 `PerformClick()` 이벤트 연결과 우클릭 메뉴 deferral 순서를 검증합니다.
...
별도 `IndianaExpedition.VisualTestHost`와 Windows Graphics Capture로 메인 화면과 사이드바, 팝업·찾기·검색 기록 삭제 상태, 다운로드 진행·완료·기록 창, 권한 요청·개인 정보 탭, 페이지 우클릭 메뉴, 도움말 메뉴 및 정보 창까지 14개 상태를 실제 PNG로 캡처할 수 있습니다. 각 캡처는 `tests/VisualBaselines`의 프로젝트 소유 기준선과 비교하고 실패 시 `artifacts/wgc/diff`에 차이 이미지를 남깁니다. 테스트 창은 포커스를 얻지 않고 뒤쪽에서 실행되며 WGC가 실패해도 `PrintWindow` 같은 방식으로 우회하지 않습니다. 준비 파일에는 호스트가 직접 만든 캡처 대상 HWND가 기록되고, 스크립트가 실행 프로세스 소유인지 검증한 뒤 캡처합니다.
...
릴리스 후보는 로컬에서 `./scripts/verify-release.ps1 -Version 0.4.0`으로 공통 검증, 로컬 WebView2 smoke와 WGC 14개 기준선 비교를 모두 통과시킨 뒤 GitHub의 수동 `Draft release` 워크플로로 생성합니다. 자동 공개는 하지 않으며 자산과 체크섬을 사람이 확인한 후 공개합니다. 전체 순서는 [릴리스 절차](docs/RELEASING.md)에 정리했습니다.
...
단일 앱 프로세스 안의 여러 브라우저 창은 같은 서비스 인스턴스와 WebView2 프로필을 안전하게 공유합니다. 동일한 데이터 디렉터리를 사용하는 Indiana Expedition 프로세스를 여러 개 동시에 실행하는 방식은 지원 범위가 아니며, 프로세스 간 잠금·충돌 병합·URL 전달 IPC는 제공하지 않습니다.
