...
0.4 품질 및 안정성
------------
- 세션과 방문 기록을 500ms 동안 합친 뒤 백그라운드 저장
- 저장 실패 재시도와 상태 표시, 종료 시 제한 시간 내 마지막 변경 flush
- 메뉴, 도구 모음, 단축키와 우클릭 메뉴가 공유하는 브라우저 명령 catalog
- 최근 주소 대소문자 중복 제거와 최대 100개 제한
- 로컬 WebView2 smoke, 비간섭 동작 테스트와 14개 WGC 기준선 회귀 검사

기존 주요 기능
------------
- 파일별 Luna 다운로드 진행 창과 일시 중지, 계속, 취소
- 실제 파일을 지우지 않는 최대 200개의 최근 다운로드 기록(Ctrl+J)
- 한 번 또는 항상 허용/차단을 선택할 수 있는 웹 사이트 권한 요청
- 인터넷 옵션의 사이트 권한 관리자와 선택형 권한 초기화

검색 기록 삭제
-------------
기본 선택은 방문 기록, 다운로드 기록, 캐시, 쿠키, 사이트 저장소입니다.
자동 완성, 저장된 암호와 웹 사이트 권한은 기본 선택되지 않습니다. 다운로드 기록을 삭제해도
...
사용자 데이터
-----------
설정, 즐겨찾기, 방문 기록, 다운로드 기록, 쿠키와 WebView2 프로필은 다음 경로에 저장됩니다.
%LocalAppData%\IndianaExpedition

한 앱 프로세스의 여러 창은 지원하지만 동일 데이터 폴더를 공유하는 여러 앱
프로세스의 동시 실행은 지원하지 않습니다.
